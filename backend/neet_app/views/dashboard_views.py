...
def dashboard_analytics(request):
    ...
    Query Parameters:
    - async: Set to 'true' to enqueue task for background processing (returns task_id)
    """
    try:
        # Check if async processing is requested
        async_flag = request.GET.get('async', '').lower() == 'true'
        
        if async_flag:
            try:
                from ..tasks import dashboard_analytics_task
                if not hasattr(request.user, 'student_id'):
                    return Response({"error": "User not properly authenticated"}, status=401)
                
                task = dashboard_analytics_task.delay(request.user.student_id)
                return Response({'status': 'queued', 'task_id': task.id}, status=202)
            except ImportError:
                # Celery not available, fall back to synchronous processing
                logger.warning('Celery not available, falling back to synchronous dashboard analytics')
            except Exception as e:
                logger.error(f'Failed to enqueue dashboard_analytics_task: {e}')
...
def dashboard_comprehensive_analytics(request):
    ...
    try:
        ...
        if async_flag:
            try:
                from ..tasks import dashboard_comprehensive_analytics_task
                if not hasattr(request.user, 'student_id'):
                    return Response({"error": "User not properly authenticated"}, status=401)
                
                task = dashboard_comprehensive_analytics_task.delay(request.user.student_id)
                return Response({'status': 'queued', 'task_id': task.id}, status=202)
            except ImportError:
                # Celery not available, fall back to synchronous processing
                logger.warning('Celery not available, falling back to synchronous comprehensive analytics')
            except Exception as e:
                logger.error(f'Failed to enqueue dashboard_comprehensive_analytics_task: {e}')
                # Fall through to synchronous processing
        ...
        logger.error(f"Error in dashboard_comprehensive_analytics: {str(e)}")
        return Response({"error": "Failed to generate comprehensive analytics"}, status=500)
...
def platform_test_analytics(request):
    ...
    Query Parameters:
    - test_id: Optional platform test ID to get specific test analytics
    - async: Set to 'true' to enqueue task for background processing (returns task_id)
    """
    try:
        # Check if async processing is requested
        ...
        if async_flag:
            try:
                from ..tasks import platform_test_analytics_task
                if not hasattr(request.user, 'student_id'):
                    return Response({"error": "User not properly authenticated"}, status=401)
                
                test_id = request.GET.get('test_id')
                task = platform_test_analytics_task.delay(request.user.student_id, test_id)
                return Response({'status': 'queued', 'task_id': task.id}, status=202)
            except ImportError:
                # Celery not available, fall back to synchronous processing
                logger.warning('Celery not available, falling back to synchronous platform test analytics')
            except Exception as e:
                logger.error(f'Failed to enqueue platform_test_analytics_task: {e}')
                # Fall through to synchronous processing
        ...
        logger.error(f"Error in platform_test_analytics: {str(e)}")
        return Response({"error": "Failed to generate platform test analytics"}, status=500)